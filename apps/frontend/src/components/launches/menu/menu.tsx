...
export const Menu: FC<{
  ...
  const menuItems = (
    <div className="flex flex-col gap-[4px] p-[8px]">
          {canDisable && !findIntegration?.refreshNeeded && (
            <div
              className="flex gap-[12px] items-center h-[48px] text-start px-[16px] rounded-[8px] hover:bg-boxHover cursor-pointer"
          ...
          >
            ...
            <div className="text-[14px] text-red-500">{t('delete', 'Delete')}</div>
          </div>
        </div>
  );
  ...
  return (
    <div
      ...
      ref={ref}
    >
      <span className="text-menuDots group-hover/profile:text-menuDotsHover text-[20px] leading-none select-none">&#x22EF;</span>
      <div>
        <div ref={showRef} />
      </div>
      {isMobile ? (
        <div onClick={(e) => e.stopPropagation()}>
          <BottomSheet
            ...
            label={t('channel_options', 'Channel Options')}
          >
            {menuItems}
          </BottomSheet>
        </div>
        ...
        show && (
          <div
            ...
            className="fixed p-[12px] bg-newBgColorInner shadow-menu flex flex-col gap-[4px] z-[100] rounded-[8px] border border-tableBorder text-nowrap"
          >
            {menuItems}
          </div>
        )
